...
for(let i: number = 0; i < sumFinalScoreEachStudent.length; i++){
    const getGrade: string = compareGrade(sumFinalScoreEachStudent[i].score.valueOf());
    const getName: string = sumFinalScoreEachStudent[i].studentName.valueOf();
    console.log(`Student ${String(i + 1)}.${getName} got : ${getGrade}`);
}
